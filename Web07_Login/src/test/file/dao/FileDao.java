...
public class FileDao {
	...
	public List<FileDto> getListW(FileDto dto){
		...
		public int getCountTF(FileDto dto) {
			...
			try {
				conn = new DbcpBean().getConn();
				//실행할 sql 문 준비하기 ROWNUM중에서 가장 큰 숫자를 얻어오면 전체 ROW의 갯수가 된다
				//혹시 ROW가 하나도 없으면 null이 얻어와지기 때문에 null인 경우를 0으로 바꿔줘야 한다(NVL(xxx, 0)
